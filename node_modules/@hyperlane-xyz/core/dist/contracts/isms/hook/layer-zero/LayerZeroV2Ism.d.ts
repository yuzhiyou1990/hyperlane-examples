...
export interface LayerZeroV2IsmInterface extends utils.Interface {
    ...
    getFunction(nameOrSignatureOrTopic: "PACKAGE_VERSION" | "PACKAGE_VERSION()" | "VERIFIED_MASK_INDEX" | "VERIFIED_MASK_INDEX()" | "authorizedHook" | "authorizedHook()" | "endpoint" | "endpoint()" | "isVerified" | "isVerified(bytes)" | "lzReceive" | "lzReceive((uint32,bytes32,uint64),bytes32,bytes,address,bytes)" | "moduleType" | "moduleType()" | "preVerifyMessage" | "preVerifyMessage(bytes32,uint256)" | "releaseValueToRecipient" | "releaseValueToRecipient(bytes)" | "setAuthorizedHook" | "setAuthorizedHook(bytes32)" | "verifiedMessages" | "verifiedMessages(bytes32)" | "verify" | "verify(bytes,bytes)"): FunctionFragment;
