...
import { DomainRoutingIsm__factory, IAggregationIsm__factory, IInterchainSecurityModule__factory, IMultisigIsm__factory, IRoutingIsm__factory, MailboxClient__factory, OPStackIsm__factory, PausableIsm__factory, StaticAggregationIsm__factory, TrustedRelayerIsm__factory, } from '@hyperlane-xyz/core';
import { deepEquals, eqAddress, formatMessage, normalizeAddress, objMap, rootLogger, } from '@hyperlane-xyz/utils';
import { IsmType, ModuleType, ismTypeToModuleType, } from './types.js';
...
export async function routingModuleDelta(destination, moduleAddress, config, multiProvider, contracts, mailbox) {
    ...
    const safeConfigDomains = objMap(config.domains, (chainName) => multiProvider.getDomainId(chainName));
    const delta = {
        domainsToUnenroll: [],
        domainsToEnroll: [],
    };
...
export function collectValidators(origin, config) {
    ...
    let validators = [];
    if (config.type === IsmType.MERKLE_ROOT_MULTISIG ||
