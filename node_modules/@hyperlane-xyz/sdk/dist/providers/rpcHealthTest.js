...
export async function isRpcHealthy(metadata, rpcIndex) {
    ...
    else if (provider.type === ProviderType.CosmJsWasm ||
        provider.type === ProviderType.CosmJs)
        return isCosmJsProviderHealthy(provider.provider, metadata);
    else
        throw new Error(`Unsupported provider type ${provider.type}, new health check required`);
}
export async function isEthersV5ProviderHealthy(provider, metadata, mailboxAddress) {
...
export async function isSolanaWeb3ProviderHealthy(provider, metadata) {
...
export async function isCosmJsProviderHealthy(provider, metadata) {
...
//# sourceMappingURL=rpcHealthTest.js.map