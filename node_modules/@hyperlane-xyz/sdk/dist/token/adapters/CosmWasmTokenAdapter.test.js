...
const multiProtocolProvider = MultiProtocolProvider.createTestMultiProtocolProvider();
const adapter = new CosmWasmCoreAdapter(testCosmosChain.name, multiProtocolProvider, neutronAddresses);
export async function getSigningClient(pkey) {
    const wallet = await DirectSecp256k1Wallet.fromKey(Buffer.from(pkey, 'hex'), testCosmosChain.bech32Prefix);
    const [account] = await wallet.getAccounts();
    const clientBase = await Tendermint37Client.connect(testCosmosChain.rpcUrls[0].http);
    const gasPrice = GasPrice.fromString('0.1untrn');
    const wasm = await SigningCosmWasmClient.createWithSigner(clientBase, wallet, {
        gasPrice,
    });
    const stargate = await SigningStargateClient.createWithSigner(clientBase, wallet, {
        gasPrice,
    });
...
export async function summary() {
    ...
    if (defaultIsmContract.label === 'hpl_ism_multisig') {
        const multisigAdapter = new CosmWasmMultisigAdapter(testCosmosChain.name, multiProtocolProvider, { multisig: defaultIsm });
...
export async function rotateValidators() {
    const multisigAdapter = new CosmWasmMultisigAdapter(testCosmosChain.name, multiProtocolProvider, {
