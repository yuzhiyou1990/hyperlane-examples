...
import { HypERC20Collateral__factory, HypERC20__factory, HypERC4626Collateral__factory, HypERC4626OwnerCollateral__factory, HypERC4626__factory, ProxyAdmin__factory, TokenRouter__factory, } from '@hyperlane-xyz/core';
...
export class EvmERC20WarpRouteReader extends HyperlaneReader {
     ...
     */
    async fetchTokenConfig(type, tokenAddress) {
        if (type === TokenType.collateral ||
            type === TokenType.collateralVault ||
            type === TokenType.collateralVaultRebase) {
            const erc20 = HypERC20Collateral__factory.connect(tokenAddress, this.provider);
            const token = await erc20.wrappedToken();
            const { name, symbol, decimals, totalSupply } = await this.fetchERC20Metadata(token);
            ...
            if (type === TokenType.syntheticRebase) {
                const hypERC4626 = HypERC4626__factory.connect(tokenAddress, this.provider);
