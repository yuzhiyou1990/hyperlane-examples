...
import { ArbL2ToL1Ism__factory, DomainRoutingHook__factory, IL1CrossDomainMessenger__factory, IPostDispatchHook__factory, InterchainGasPaymaster__factory, OPStackIsm__factory, Ownable__factory, PausableHook__factory, ProtocolFee__factory, StaticAggregationHookFactory__factory, StaticAggregationHook__factory, StorageGasOracle__factory, } from '@hyperlane-xyz/core';
import { addressToBytes32, deepEquals, eqAddress, rootLogger, } from '@hyperlane-xyz/utils';
...
import { HookConfigSchema, HookType, MUTABLE_HOOK_TYPE, } from './types.js';
...
export class EvmHookModule extends HyperlaneModule {
    ...
    async update(targetConfig) {
        ...
        switch (targetConfig.type) {
            case HookType.INTERCHAIN_GAS_PAYMASTER:
                updateTxs.push(...(await this.updateIgpHook({
                    currentConfig,
                    targetConfig,
                })));
                break;
    ...
    static async create({ chain, config, proxyFactoryFactories, coreAddresses, multiProvider, contractVerifier, }) {
    ...
    async deployRoutingHook({ config, }) {
        ...
        if (config.type === HookType.FALLBACK_ROUTING) {
            ...
            routingHook = await this.deployer.deployContract(this.chain, HookType.FALLBACK_ROUTING, [this.args.addresses.mailbox, deployerAddress, fallbackHook.address]);
        }
        else {
            // deploy routing hook
