...
export function parseWarpRouteMessage(messageBody) {
    const RECIPIENT_OFFSET = 0;
    const AMOUNT_OFFSET = 32;
    const buf = fromHexString(messageBody);
    const recipient = toHexString(buf.subarray(RECIPIENT_OFFSET, RECIPIENT_OFFSET + 32));
    const amount = BigInt(toHexString(buf.subarray(AMOUNT_OFFSET, AMOUNT_OFFSET + 32)));
    return {
        recipient,
        amount,
    };
